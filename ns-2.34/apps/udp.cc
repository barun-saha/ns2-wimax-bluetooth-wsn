...
static const char rcsid[] =
    "@(#) $Header: /home/rouil/cvsroot/ns-2.34/apps/udp.cc,v 1.1.1.1 2008/04/11 18:40:29 rouil Exp $ (Xerox)";
#endif
...
void UdpAgent::sendmsg(int nbytes, AppData* data, const char* flags)
{
	...
	while (n-- > 0) {
		//p = allocpkt();//commented for mannasim--ARIJIT
		// Alterated by The Manna Research Group	//Added for mannasim--ARIJIT
       		// Used by mannasim - wireless sensor networks simulator.   //Added for mannasim--ARIJIT
		// p = allocpkt(); 	//Added for mannasim--ARIJIT
		p = allocpkt(sizeof(data));	//Added for mannasim--ARIJIT
	...
	n = nbytes % size_;
	if (n > 0) {
		//p = allocpkt();	//Deleted for mannasim--ARIJIT
		//p = allocpkt(); Alterado pelo grupo Manna	//Added for mannasim--ARIJIT
		p = allocpkt(sizeof(data));	//Added for mannasim--ARIJIT
