...
#define NODE_FORMAT	"$ns_ at %.12f \"$%s(%d) setdest %.12f %.12f %.12f\"\n"
#define NODE_FORMAT2	"$%s(%d) setdest %.12f %.12f %.12f\n"
#define NODE_FORMAT3	"$%s(%d) set %c_ %.12f\n"
#define NODE_FORMAT4	"$%s(%d) pos %.12f %.12f\n"
#define NODE_FORMAT5	"$%s(%d) on\n"
#define NODE_FORMAT6	"$ns_ at %.12f \"$%s(%d) on\"\n"
...
main(int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "c:n:m:p:s:t:x:y:i:o:")) != EOF) {       
...
Node::Node()
{
	...
	if (STARTTIME > 0) {
         ...
         * are stationary.
         */
	time_arrival = t + PAUSE;
	time_update = t;
	time_transition = 0.0;
	if (STARTTIME >= 0) {
		fprintf(stdout, NODE_FORMAT6, t, NNAME, index);
	}
	...
	fprintf(stdout, NODE_FORMAT4, NNAME, index, position.X, position.Y);
...
Node::Update()
{
	...
	if(TIME == time_arrival) {
		...
		fprintf(stdout, NODE_FORMAT,
			TIME, NNAME, index, destination.X, destination.Y, speed);
