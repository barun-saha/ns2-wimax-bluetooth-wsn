...
class ThroughputWatch: public StatWatch  {
 public:
  ...
  threshold_action_t update(double size, double time) {
    ...
    old_average_ = average_;
    if(time_.average() !=0)  // RPI - added because of floating point exception--  time_.average() was probbaly becoming 0 because multiple packets are going at the same time coz of OFDMA . 
