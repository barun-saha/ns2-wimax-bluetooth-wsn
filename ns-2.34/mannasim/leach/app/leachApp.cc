...
static class LeachAppClass : public TclClass
{
        public:
			...
			// Added by Barun
			//printf("[Debug:leachApp.cc] nClusters: %d\n", atoi(argv[5]));
			// End changes
                        return new LeachApp(
                                atoi(argv[4]),
                                atoi(argv[5]),
...
{
    // Commented for ANT VLabs: Barun, 27Aug2011
...
{
	isCH_      = false;
	// Added by Barun -- shouldn't it be set to false if it doesnt become CH???
	//hasBeenCH_ = false;	
	// End changes
}
...
void LeachApp::decideClusterHead()
{
	...
	if (round_ >= totRounds) {
		// Added by Barun
		//printf("[Debug:leachApp.cc] Current round_ %d; resetting round_ to 0 for node #%d...\n", round_, sensor_node_->nodeid());
		// End changes
		round_ = 0;	
	}
	...
	double random_nbr = Random::uniform(0, 1);
	double cal_pi = calculatePi();
	//printf("[Debug:leachApp.cc] Node #%d, Random #: %lf, T(n): %lf\n", sensor_node_->nodeid(), random_nbr, cal_pi);
	if (random_nbr < cal_pi)
	{
...
double LeachApp::calculatePi()
{
	...
	if (hasBeenClusterHead()) {
		...
		// Added by Barun
		//printf("[Debug:leachApp.cc] Node #%d, Round# %d:: P: k (%d) / n (%d) = %lf\n", sensor_node_->nodeid(), round_, k, n, k*1.0/n);
		// End changes
...
{
	// Added by Barun
	//printf("[Debug: leachApp.cc] msg->msgSize(): %d\n", msg->msgSize());
	// End changes
	send(
		mac_dst,
		link_dst,
...
void LeachApp::sendDataToBS()
{
	...
	if (sensedData_->count() > 0)
	{
		...
		sensedData_->node_id() = sensor_node_->nodeid();
		//Added by Barun
		//printf("[Debug: leachApp.cc] B4 sending ... %d \n", dataSize);
		// End
		send(
			MAC_BROADCAST,
			destination_id_,
