...
#check input parameters
    ...
    puts "Usage: ns datarate.tcl modulation x_dist cyclic_prefix "
    puts "modulation: 1-7"
    ...
    puts "x_dist: Distance of the mobile node from the base station (in metre))"
...
set x_dist [lindex $argv 1]

#define coverage area for base station: 500m coverage 
Phy/WirelessPhy/OFDMA set g_ [lindex $argv 2]
# Trying to increase coverage area
#Phy/WirelessPhy set Pt_ 0.2 
Phy/WirelessPhy set Pt_ 0.222
...
set opt(x)		5100			   ;# X dimension of the topography
set opt(y)		5100			   ;# Y dimension of the topography

set opt(base_stn_x)     100         ;# x-coordinate of base station
...
#open file for trace
set tf [open wimax_mod_$diuc\_$x_dist.tr w]
$ns trace-all $tf
...
$bstation set X_ $opt(base_stn_x)
...
$wl_node set X_ [expr $opt(base_stn_x) + $x_dist]
